...
export default function ProjectView() {
  ...
  const { id } = useParams();
  const [info, setInfo] = useState<ProjectResponseProps | null>(
    ResponseProjectData,
  );
          ...
          <div className="w-full flex flex-col gap-5">
            <div className="flex justify-end">
              <div>
                <button
                  onClick={() =>
                    navigate(`/client/ticket/form/${id}`, { replace: true })
                  }
                  className="flex items-center gap-2 disabled:bg-blue-400 bg-primary hover:bg-secondary rounded-sm  text-white p-2 text-[14px]"
                >
                  <BiPlus />
                  <p>Create Ticket</p>
                </button>
            ...
            <TicketTableHeader>
              {info.tickets &&
                info.tickets.map((items, index) => (
                  <TicketTableData 
                  onNavigate={(ticketID : number) => navigate(`/client/ticket/form/${id}/${ticketID}`)}
                  items={items} 
                  count={++index}
                   />
                ))}
